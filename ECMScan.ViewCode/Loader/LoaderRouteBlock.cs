...
namespace ISBLScan.ViewCode
{
  ...
  public class RouteBlock : LoaderCommon
  {
    ...
    public void Load(List<IsbNode> isblList)
    {
      ...
      SqlCommand command = new SqlCommand();
      command.Connection = Connection;
      command.CommandText = "select XRecID, Name, Comment, Properties, LastUpdate, BlockGroup from SBRouteBlock order by Name";
      command.Prepare();
      SqlDataReader reader = command.ExecuteReader();
      if (reader.HasRows)
      {
        while (reader.Read())
        {
          ...
          listGroups.First(n => n.Id == groupId).Nodes.Add(routeBlockNode);
        }
      }
      reader.Close();
